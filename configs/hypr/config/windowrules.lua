...
hl.window_rule({ match = { float = false }, center = true })
...
hl.window_rule({ match = { class = "^(.*[Cc]alculator.*)$" }, float = false, size = { "max(monitor_w, monitor_h)*0.17", "min(monitor_w, monitor_h)*0.43" } })
...
hl.window_rule({ match = { class = "^(firefox|app.zen_broswer.zen)$" }, opacity = "0.90 0.90 override" })
hl.window_rule({ match = { class = "^(steam)$" }, opacity = "0.90 override" })
...
-- HotkeyHub (keybind cheatsheet)
hl.window_rule({
    name = "hotkeyhub-float",
